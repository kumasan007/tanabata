...
create trigger atomic_test_subcompany after insert or update or delete on public.schedule_subcompanies
  for each row execute function pg_temp.track_atomic_test_operation();
create trigger atomic_test_vehicle after insert or update or delete on public.schedule_aerial_work_vehicles
  for each row execute function pg_temp.track_atomic_test_operation();
...
declare
  ...
  vehicles jsonb := '[{"work_area":"10F","vehicle_count":2}]';
  result jsonb;
  original jsonb;
  current_state jsonb;
  rejected boolean;
...
begin
  groups := jsonb_build_array(
    jsonb_build_object('work_date','2026-09-15','primary_company',company_name,'primary_count',1,
      'work_area','10F','work_content','original','aerial_work_vehicle_count',2,
      'aerial_work_vehicle_floor','10F','uses_fire',true,'uses_tachiuma',true,'tachiuma_notes','2'),
    jsonb_build_object('work_date','2026-09-16','primary_company',company_name,'primary_count',1,
      'work_area','10F','work_content','original','aerial_work_vehicle_count',2,
      'aerial_work_vehicle_floor','10F','uses_fire',true,'uses_tachiuma',true,'tachiuma_notes','2')
  );
  result := public.save_schedule_atomically(groups, subs, vehicles);
  ...
  if (select count(*) from public.schedule_subcompanies where schedule_group_id in (
    select id from public.schedule_groups where primary_company = company_name)) <> 2
    or (select count(*) from public.schedule_aerial_work_vehicles where schedule_group_id in (
  ...
  select jsonb_agg(to_jsonb(s) || jsonb_build_object(
    ...
    'vehicles', (select jsonb_agg(to_jsonb(v) order by v.id) from public.schedule_aerial_work_vehicles v where v.schedule_group_id = s.id)
  ) order by s.work_date) into original from public.schedule_groups s where primary_company = company_name;

  -- 車両のinsertで失敗しても、親の更新・子の削除を全て取り消す。
  rejected := false;
  begin
    perform public.save_schedule_atomically(jsonb_set(groups, '{0,work_content}', '"changed"'),
      subs, '[{"work_area":"10F","vehicle_count":0}]', true);
  exception when check_violation then rejected := true;
  end;
  if not rejected then raise exception 'Invalid vehicle was accepted'; end if;

  -- 2日目の検証で失敗しても、1日目の保存を取り消す。
  rejected := false;
  begin
    perform public.save_schedule_atomically(
      jsonb_set(jsonb_set(groups, '{0,work_content}', '"changed"'), '{1,primary_count}', '-1'), subs, vehicles, true);
  ...
  rejected := false;
  begin
    perform public.save_schedule_atomically(groups, subs, vehicles);
  ...
  result := public.save_schedule_atomically(groups || jsonb_build_array(
    jsonb_set(groups->0, '{work_date}', '"2026-09-17"')), subs, vehicles, false, true);
  if result->'dates' <> '["2026-09-17"]'::jsonb then raise exception 'Skip-existing dates are incorrect'; end if;

  rejected := false;
  begin
    perform public.save_schedule_atomically(jsonb_build_array(groups->0), subs, vehicles, true, false, gen_random_uuid());
  exception when raise_exception then
  ...
  if (select work_content from public.schedule_groups where id = (result->'savedIds'->>0)::uuid) <> 'updated'
    or exists (select 1 from public.schedule_subcompanies where schedule_group_id = (result->'savedIds'->>0)::uuid)
    or exists (select 1 from public.schedule_aerial_work_vehicles where schedule_group_id = (result->'savedIds'->>0)::uuid) then
