...
export function AdminDashboard({ initialAuthenticated }: { initialAuthenticated: boolean }) {
    ...
    const [recoveryTab, setRecoveryTab] = useState<"backups" | "auditLogs" | null>(null);
    ...
    return <main className="admin-dashboard min-h-screen bg-[#f6f7f5]"><div className="mx-auto grid max-w-6xl gap-3 px-4 py-3 sm:px-6 sm:py-4"><div className="flex justify-end"><button className="btn btn-secondary px-3" type="button" disabled={logoutLoading} onClick={() => void logout()}><LogOut size={17} aria-hidden="true"/>ログアウト</button></div>{message && <p role="alert" className="notice-error text-sm">{message}</p>}<AdminTabs active={activeTab} onChange={(tab) => { setActiveTab(tab); if (tab !== "companies") setRecoveryTab(tab); }}/><div hidden={activeTab !== "companies"}><CompaniesPanel refreshVersion={companyVersion}/></div>{recoveryTab && <div hidden={activeTab === "companies"}><RecoveryPanel kind={recoveryTab} active={activeTab !== "companies"} onRestored={() => setCompanyVersion(v => v + 1)}/></div>}</div></main>;
}
