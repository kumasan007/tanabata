...
会社名リストはSupabaseの `company_master` テーブルで管理します。管理画面の「協力会社一覧」タブから追加・編集・並び替え・削除でき、職人側フォームへ即時反映されます。追加時は一次会社を1回入力し、複数の二次会社を1行ずつまとめて登録できます。並び替えは一次会社を選択し、配下の二次会社を含むグループ単位で行います。UUIDは行の内部識別にだけ使い、画面には表示しません。
